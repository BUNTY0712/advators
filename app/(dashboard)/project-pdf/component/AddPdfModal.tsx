...
export function AddPdfModal({ projectId , refetch }: any) {
  const [isOpen, setIsOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [fileName, setFileName] = useState("");
  const [assetType, setAssetType] = useState("basic"); // ✅ NEW
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("asset_type", assetType); // ✅ dynamic
      ...
      setAssetType("basic");
      setIsOpen(false);
      refetch(); // ✅ refresh list (if passed)
