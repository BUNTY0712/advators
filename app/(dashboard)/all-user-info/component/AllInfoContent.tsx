...
import LicenseDetails from "../component/LicenseDetails";
...
export default async function AllInfoContent() {
  ...
  {tab}
        ...
        {TABS.map((tab) => (
          <TabsContent key={tab} value={tab} className="bg-white mt-5 max-w-4xl p-5 border rounded-md shadow-sm">
            {tab === "Basic Details" && <BasicDetails id={id} />}
            {tab === "License Details" && <LicenseDetails id={id} />}
            {tab === "Account Details" && <AccountDetails id={id} />}
            {tab === "Visa Details" && <VisaDetails id={id}/>}
            {tab === "Contract Details" && <ContractDetails id={id}/>}
            {tab === "Documents" && <div className="p-4 text-gray-500">Documents Upload</div>}
          </TabsContent>
        ))}
      </Tabs>
    </div>
