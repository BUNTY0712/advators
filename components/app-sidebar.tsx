...
const iconMap: Record<string, React.ReactNode> = {
  "Create Client": <Users size={16} />,
  "Create Project": <Briefcase size={16} />,
  "RCTI": <FileText size={16} />,
  "Additional Info": <Info size={16} />,
  "Tracking": <MapPin size={16} />,
}

const menuItems = [
  { label: "Create Project", href: "/all-projects" },
  { label: "Create Client", href: "/all-clients" },
  { label: "Create Employee", href: "/all-employees" },
  { label: "Create Dealer", href: "/all-dealers" },
  { label: "Create Location", href: "/all-locations" },


  // { label: "All Drivers", href: "/all-drivers" },
  // { label: "All Jobs", href: "/all-jobs" },
  // { label: "RCTI", href: "/rcti" },
...
const subMenuItems = [
   { label: "Basic Info", href: "/basic-info" },
  // { label: "License Info", href: "/license-info" },
  // { label: "Visa Info", href: "/all-drivers" },
...
>
  ...
  <SidebarMenuItem key={item.label}>
    <SidebarMenuButton asChild>
      <Link
        href={item.href}
        className={`wtm-nav-item ${
          pathname === item.href ||

          // ✅ PROJECT GROUP
          (item.href === "/all-projects" &&
            (
              pathname.startsWith("/project-gallery") ||
              pathname.startsWith("/project-pdf") ||
              pathname.startsWith("/project-excel") ||
              pathname.startsWith("/project-view") ||
              pathname.startsWith("/project-dealer")
            )
          ) ||

          // ✅ DEALER GROUP (ADDED)
          (item.href === "/all-dealers" &&
            (
              pathname.startsWith("/dealer-view")
            )
          )
      ...
      >
        <span className="wtm-icon">
          {iconMap[item.label] || <LayoutDashboard size={16} />}
        </span>
